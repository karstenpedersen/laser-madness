...
///@func effect_floor_layer(sprite, index, x, y, scale, angle, color, alpha
function effect_floor_layer(_sprite, _subimg, _x, _y, _scale, _angle, _color, _alpha) {
	with (cnt_draw) {
		if (surface_exists(floor_surface)) {
			_color = merge_color(_color, c_dkgray, 0.125);
			surface_set_target(floor_surface);
			draw_sprite_ext(_sprite, _subimg, _x,_y, _scale, _scale, _angle, _color, _alpha);
